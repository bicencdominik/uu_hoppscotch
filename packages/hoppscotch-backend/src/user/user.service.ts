...
export class UserService {
  ...
  /**
   * Create a new User with a local password, or update the password of an
   * existing one.
   *
   * Used by the boot-time admin seeder and by the create-user CLI. Both go
   * through here so that hashing parameters and the Account bookkeeping stay
   * identical no matter how the account was made.
   *
   * @param username Login identifier, stored in the email column
   * @param passwordHash Hash produced by `hashPassword` -- never a plaintext password
   * @param isAdmin Whether the account should be an admin. Leave undefined when
   *        updating to preserve the existing role; a bare password rotation must
   *        not silently demote an administrator.
   * @returns The created or updated User
   */
  async upsertUserWithPassword(
    username: string,
    passwordHash: string,
    isAdmin?: boolean,
  ) {
    const existingUser = await this.findUserByEmail(username);
